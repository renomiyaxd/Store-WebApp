...
class Cart {
    ...
    public function insert($item = array()){
        if(!is_array($item) OR count($item) === 0){
            ...
            if(!isset($item['id'], $item['productName'],$item['productVariant'], $item['price'], $item['qty'])){
                return FALSE;
            }else{
                /*
                 * Insert Item
    ...
    public function update($item = array()){
        if (!is_array($item) OR count($item) === 0){
            ...
            if (!isset($item['id'], $this->cart_contents[$item['rowid']])){
                return FALSE;
            }else{
                // prep the quantity
                if(isset($item['qty'])){
                ...
                // find updatable keys
                $keys = array_intersect(array_keys($this->cart_contents[$item['id']]), array_keys($item));
                // prep the price
                if(isset($item['price'])){
                    $item['price'] = (float) $item['price'];
                }
                // product id & name shouldn't be changed
                foreach(array_diff($keys, array('id', 'productName')) as $key){
                    $this->cart_contents[$item['id']][$key] = $item[$key];
                }
                // save cart data
                $this->save_cart();
                return TRUE;
